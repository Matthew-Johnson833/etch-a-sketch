...
// initially only create a 16x16 grid, adjustable later.
function generateDivs() {
    let container = document.querySelector("#container");

    
    for (let i = 0; i < 16/* change to variable here */; i++) {
        let row = document.createElement('div'); // every time i increments, create a new div called row
        row.classList.add("row");
        for (let x = 1; x <= 16 /* same variable */; x++) {
            ...
            cell.innerText = (i * 16/* same variable */) + x; // remove, this is a counter for how many cells created
            row.appendChild(cell); // add to the row
        }
        container.appendChild(row); // add the completed row to the container
    }
...
function changeColor() {
    // fancy code creation by people way smarter than I, -- https://www.paulirish.com/2009/random-hex-color-code-snippets/
    ...
    colorChange.style.backgroundColor = color; // typeError -- this is null ??
}
